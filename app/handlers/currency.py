...
async def currency1_chosen(message: types.Message, state: FSMContext):
    ...
    for size in available_currency["data"][user_data['chose_currency1']]:
        keyboard.add(size)
    await Currency.next()
    await message.answer("Теперь выберите вторую валюту :", reply_markup=keyboard)
...
async def currency2_chosen(message: types.Message, state: FSMContext):
    ...
    user_data = await state.get_data()
    available_currency = available_currency["data"][user_data['chose_currency1']][user_data['chose_currency2']]
    await message.answer(f"Курс обмена {user_data['chose_currency1']} на {user_data['chose_currency2']}:\n"
                         f"Покупка - {available_currency['in']}\n"
                         f"Продажа - {available_currency['out']}", reply_markup=types.ReplyKeyboardRemove())
    await state.finish()
